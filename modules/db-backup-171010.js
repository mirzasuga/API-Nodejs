    ...
    var functions = {
	    ...
	    getCurrentCampaign(params,cb){
            ...
            getFromDriverId:function(params,cb){
                var query = 'select *, up.email_id as email, ifnull(c.campaign_name,"") as campaign_name, c.start_date, c.end_date, ifnull(c.id,0) as campaign_id, up.id as driver_id, cr.wrap_start_date \
                        from stick_earn.user_profile up \
                        left join stick_earn.vehicle v on up.id = v.fk_user_id \
                        left join stick_earn.campaign_request cr on cr.driver_id = up.id  \
                        left join stick_earn.campaign c on c.id = cr.campaign_id\
            ...
            callSpUpdateProfile:function(params,cb){
                var query = 'call stickearn_v2.api_update_driver_profile(:driver_id, :reg_number, :mobile_number, :bank_acc_number, :img_kendaraan, :img_sim, :img_stnk,  :img_tabungan )';
                statement(query,params,cb);
            },
